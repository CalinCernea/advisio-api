...
def _get_or_create_folder(service, folder_name: str, parent_id: str) -> str:
    ...
    # Căutare folder cu parametri necesari pentru Service Account
...
def upload_to_drive(pdf_bytes: bytes, filename: str, folder_name: str) -> str:
    ...
    # Obține sau creează sub-folderul restaurantului
    folder_id = _get_or_create_folder(service, folder_name, PARENT_FOLDER)

    # Șterge versiunea veche dacă există (evită duplicate)
    delete_query = f"name='{filename}' and '{folder_id}' in parents and trashed=false"
